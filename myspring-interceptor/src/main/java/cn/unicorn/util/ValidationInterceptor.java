...
public class ValidationInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (handler instanceof HandlerMethod) {// 执行向下转型前应该首先判断其是否是指定类的实例
			...
			String validationRuleKey = handlerMethod.getBeanType().getSimpleName() + "."
			...
			try {// 如果指定的key不存在,表示现在不需要进行验证
				...
				if (validationRule != null) {// 此时有验证处理操作,则需要进行验证处理
					...
					if (avu.getErrors().size() > 0) {// 现在有错误信息
						System.err.println(avu.getErrors());
						return false;
					}
				}
			}
			...
			// this.logger.info("@@[Action处理方法参数 - " + i + "]" +
			// handlerMethod.getMethodParameters()[i]);
			// }
		}
